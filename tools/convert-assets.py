...
def key_black(im: Image.Image) -> Image.Image:
    ...
    this 8-bit art). Removes the background EVERYWHERE — the border AND enclosed
    pockets (between robot legs, etc.) — while keeping every figure pixel, even
    dark ones (robot seams/visor, outlines) that use OTHER indices."""
    if im.mode == "P":
