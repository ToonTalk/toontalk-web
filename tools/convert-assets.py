...
def key_black(im: Image.Image) -> Image.Image:
    """Make the black background transparent — but ONLY black connected to the
    border (flood-fill), so dark pixels inside the figure (robot seams, outlines,
    shading) are kept rather than punched into 'moth-eaten' holes."""
...
def _despeckle(im: Image.Image, passes: int = 2, min_opaque: int = 4) -> Image.Image:
