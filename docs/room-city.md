...
the tool pointing out toward the work. The held clay is drawn at **roughly its
Lego size** (its at-rest footprint) so it doesn't balloon in the hand, and the
hand grips the BODY's far side (`DragController.heldHandOffset` = `1.6·heldVec`,
i.e. past the tool centre toward its far edge) so the hose/nose stays clear of the
hand. The held clay carries **no mode plate and no stomach count** — those
