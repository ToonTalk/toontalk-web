...
| Robots | ✅ | train (drop a box on it) in a **full-screen thought bubble**: imagines on a copy, **you control the robot (no hand)**, hole→hole/take-out/put-in/wand-copy/Dusty erase+suck, Bammer on combine · **runs as a guarded loop**: iterates while matching, stops on mismatch (scale guard) or grab, **waits** on an incomplete box / empty nest and resumes when filled ✅ · **run re-enacts training** (walks to Tooly, fetches a fresh element, drops it, walks home) ✅ · **nest-transparent** match + actions ✅ · generalize: per-hole erase in bubble (partial) or Dusty-erase the robot (all guards) ✅ · match/teams/copy/module-recursion ✅ · Pumpy-resize-step ▢ · non-recursive (nested-box) matching ⚠ · negation/wand-S self-copy/re-open-to-edit ▢ |
| Scale | ✅ | `<` `>` `=` guards work |
