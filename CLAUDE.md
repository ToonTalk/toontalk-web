...
| Robots | ✅ | train (drop a box on it): hole→hole + take-out + put-in by example, live thought bubble ✅ (manual-audited)/match/teams/copy/module-recursion ✅ · tool-use mid-demo ▢ · non-recursive matching ⚠ · wait-on-nest/negation/wand-S self-copy ▢ |
| Scale | ✅ | `<` `>` `=` guards work |
