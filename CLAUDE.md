...
| Robots | ✅ | train (drop a box on it) in a **full-screen thought bubble**: imagines on a copy (real box unchanged), **you control the robot (no hand)**, hole→hole/take-out/put-in (on drop)/wand-copy/Dusty erase+suck, Bammer on combine, **animated step-by-step replay** ✅ (source-audited) · match/teams/copy/module-recursion ✅ · Pumpy-resize-step ▢ · non-recursive matching ⚠ · wait-on-nest/negation/wand-S self-copy ▢ |
| Scale | ✅ | `<` `>` `=` guards work |
